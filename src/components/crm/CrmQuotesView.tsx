  ...
  const filtered = useMemo(() => {
    return quotes.filter((q) => {
      ...
      if (search) {
        ...
        const hay = [q.customer_name, q.project_arena, q.product, q.quote_number, q.comment].join(' ').toLowerCase();
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/50 text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                {['Datum', 'Uppd.', 'Säljare', 'Kund', 'Land', 'Offert nr', 'Produkt', 'Antal/Spec', 'Lev. tid', 'Föresk.', 'Sannol.', 'Belopp', 'Ansvarig', 'Uppföljning', 'Status', 'Kommentar'].map((h) => (
                  <th key={h} className="px-3 py-2 text-left font-medium whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading && <tr><td colSpan={16} className="p-8 text-center text-muted-foreground">Laddar…</td></tr>}
              {!loading && filtered.length === 0 && <tr><td colSpan={16} className="p-8 text-center text-muted-foreground">Inga offerter matchar filtren.</td></tr>}
              {filtered.map((q) => (
                <tr
                  key={q.id}
                  onClick={() => openEdit(q)}
