...
import { Plus, Mail, Phone, Building2, MapPin, Trash2 } from 'lucide-react';
...
import { COUNTRIES, formatSEK } from '@/lib/crmConstants';
...
export function CrmCustomersView() {
  ...
  const [form, setForm] = useState({ name: '', arena: '', country: 'Sverige', notes: '' });
  const [newContact, setNewContact] = useState({ name: '', email: '', phone: '', role: '' });

  const openNew = () => { setEditing(null); setForm({ name: '', arena: '', country: 'Sverige', notes: '' }); setOpen(true); };
  const openEdit = (c: CrmCustomer) => { setEditing(c); setForm({ name: c.name, arena: c.arena, country: c.country, notes: c.notes }); setOpen(true); };

  const save = async () => {
    if (!form.name.trim()) return toast.error('Namn krävs');
    const res = editing
      ? await supabase.from('crm_customers').update(form).eq('id', editing.id)
      : await supabase.from('crm_customers').insert(form);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 space-y-4">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editing ? 'Redigera kund' : 'Ny kund'}</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div><Label>Kundnamn *</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
            <div><Label>Arena / Anläggning</Label><Input value={form.arena} onChange={(e) => setForm({ ...form, arena: e.target.value })} /></div>
            <div>
              <Label>Land</Label>
            ...
            <div><Label>Anteckningar</Label><Input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></div>
          </div>
          <DialogFooter>
